...
df$THREE <- as.factor(ifelse((df$FTHG == 3 & df$FTAG == 1) |
                              (df$FTHG == 1 & df$FTAG == 3), "1", "0"))
...
split <- createDataPartition(df$THREE, p = 0.7, list = F)
...
threeone <- h2o.automl(x = x, y = y, training_frame = train_h2o,
                   nfolds = 5, stopping_metric = "AUC", max_runtime_secs = ((60*60)*6),
                   seed = 1234)

threeone@leaderboard
threeone@leader
h2o.varimp_plot(threeone@leader)
h2o.download_mojo(threeone@leader, get_genmodel_jar = TRUE)
